...
TextStyle::TextStyle(const std::string& font, const Color& color,
                     TEXT_SHADOW_STYLE style,
                     int32_t shadow_x, int32_t shadow_y)
{
    _font = font;
    _color = color;
    _shadow_style = style;
...
TextSupervisor::~TextSupervisor()
{
    ...
    for (std::map<std::string, FontProperties *>::iterator it = _font_map.begin();
         it != _font_map.end(); ++it)
        delete it->second;
                               ...
                               uint32_t font_size)
{
    ...
    bool reload = false;
    std::map<std::string, FontProperties *>::iterator it =
      _font_map.find(textstyle_name);
    if(it != _font_map.end()) {
        reload = true;
