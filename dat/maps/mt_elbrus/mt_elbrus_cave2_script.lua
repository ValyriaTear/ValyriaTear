...
function _CreateObjects()
    ...
    -- exit 2
    Map:AddHalo("img/misc/lights/torch_light_mask.lua", 0, 22,
        vt_video.Color(0.3, 0.3, 0.46, 0.8));

    -- Stones
    object = CreateObject(Map, "Rock1", 49.0, 30.3);
    Map:AddGroundObject(object);
    object = CreateObject(Map, "Rock2", 5, 16);
    Map:AddGroundObject(object);
    object = CreateObject(Map, "Rock2", 5, 18);
    Map:AddGroundObject(object);
    object = CreateObject(Map, "Rock2", 5, 20);
    Map:AddGroundObject(object);
    ...
    object = vt_map.TriggerObject("try maze trigger 1",
    ...
    object:SetPosition(49, 28);
    Map:AddFlatGroundObject(object);

    object = vt_map.TriggerObject("try maze trigger 2",
                             "img/sprites/map/triggers/rolling_stone_trigger1_off.lua",
                             "img/sprites/map/triggers/rolling_stone_trigger1_on.lua",
                             "",
                             "");
    object:SetObjectID(Map.object_supervisor:GenerateObjectID());
    object:SetPosition(21, 16);
    Map:AddFlatGroundObject(object);
...
map_functions = {
    ...
    place_shroom_after_fight = function(shroom)
        ...
        if (hero_y > shroom_y + 0.3) then
            ...
            -- don't move it in that case, the shroom is against a wall.
